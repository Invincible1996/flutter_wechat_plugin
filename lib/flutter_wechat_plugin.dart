...
class FlutterWechatPlugin {
  ...
  static Stream<Map<String, dynamic>> get responseEventHandler => _eventController.stream;
  ...
  Future<bool> shareNetworkImageToScene(String imageUrl, int scene) {
    return FlutterWechatPluginPlatform.instance.shareNetworkImageToScene(imageUrl, scene);
  }
  ...
  Future<bool> shareLink({required String url, required String title, required String description}) {
    return FlutterWechatPluginPlatform.instance.shareLink(url: url, title: title, description: description);
  }

  Future<bool> openMiniProgram({required String username, required String path, WXMiniProgramType miniProgramType = WXMiniProgramType.release}) {
    return FlutterWechatPluginPlatform.instance.openMiniProgram(username: username, path: path, miniProgramType: miniProgramType.value);
  }
}
